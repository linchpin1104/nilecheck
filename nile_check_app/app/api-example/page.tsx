...
export default function ApiExamplePage() {
  ...
  // If we're on the server, don't try to render the full page
  if (!isClient) {
    return <div className="p-8">Loading API Example...</div>;
  }
  
  return <ApiExampleContent />;
}

// Move the main component to a client-only component
function ApiExampleContent() {
  const { user } = useAuth();
  ...
  }, [user, fetchData]); // Added fetchData to dependency array to fix ESLint warning
